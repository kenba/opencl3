...
An OpenCL application must have at least one context. An OpenCL application may
create more than one context but OpenCL does not support synchronisation between
multiple contexts.
...
transferred to and from the host to the OpenCL device memory, see Figure 3.

![OpenCL Memory Model](images/opencl_memory.png)  
*Figure 3 An OpenCL Context's Memory Model*
...
The library stores OpenCL kernels by name in a [HashMap](https://doc.rust-lang.org/beta/std/collections/struct.HashMap.html) in the Context struct. An application can get
OpenCL kernels it requires by name from the Context.
...
simultaneously on multiple queues.
